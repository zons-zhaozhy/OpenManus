...
class PerformanceSettings(BaseModel):
    ...
    global_timeout: float = Field(default=60.0, description="全局超时时间(秒)")
    max_timeout: float = Field(default=180.0, description="最大超时时间(秒)")
    inactive_tolerance: float = Field(default=15.0, description="无响应容忍时间(秒)")
    response_check_interval: float = Field(default=5.0, description="响应检查间隔(秒)")
    llm_concurrent_limit: int = Field(default=2, description="LLM并发调用限制")
    circuit_failure_threshold: int = Field(default=3, description="断路器失败阈值")
    circuit_recovery_timeout: float = Field(
        default=30.0, description="断路器恢复时间(秒)"
    )
    enable_sandbox_cleanup: bool = Field(default=False, description="是否启用沙盒清理")
...
class AppConfig(BaseModel):
    ...
    performance_config: Optional[PerformanceSettings] = Field(
        None, description="Performance control configuration"
    )
...
class Config:
    ...
    def get_llm_config(self, config_name: str = "default") -> Dict[str, Any]:
        """Get LLM configuration"""
        return self.llm_configs.get(config_name, self.llm_configs["default"])
    ...
    def _load_initial_config(self):
        ...
        performance_config = raw_config.get("performance")
        if performance_config:
            performance_settings = PerformanceSettings(**performance_config)
        else:
    ...
    @property
    def performance_config(self) -> PerformanceSettings:
        """Get the Performance configuration"""
        return self._config.performance_config
...
settings = config  # Make settings available as a global variable
